...
# Sizin oluşturduğunuz protokol dosyasını dahil ediyoruz
from actions.protocol import power_on_protocol 
...
# Kesme komutları Türkçeleştirildi
interrupt_commands = ["sustur", "kapat", "çık", "dur"]
...
async def ai_loop(ui: JarvisUI):
    while True:
        ...
        # --- UMUT VE İLHAN: ARA SORU BYPASS ---
        if any(k in user_text.lower() for k in ["umut", "ilhan", "kim yaptı"]):
             power_on_protocol(ui)
             continue

        # Arayüz log kaydı
        ui.write_log(f"Siz: {user_text}")
        ...
        try:
            ...
            ui.write_log(f"YAPAY ZEKA HATASI: {e}")
            continue
        ...
        if intent == "send_message":
            ...
            if response:
                ui.write_log(f"Asistan: {response}")
...
def main():
    ...
    def runner():
        # --- SUNUM BAŞLANGICI ---
        # Program açılır açılmaz protocol.py içindeki fonksiyonu çağırır
        power_on_protocol(ui)
